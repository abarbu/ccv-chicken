...
(use traversal bind easyffi lolevel matchable define-structure linear-algebra imlib2)
         ...
         (match-let
          (((min1 min-keypoint min2)
            (let loop ((min1 +inf.0) (min-keypoint #f) (min2 +inf.0)
             ...
             (if (null? keypoints)
                 (list min1 min-keypoint min2)
                 (let* 
                   ((k (sift-descriptor (car keypoints)))
                    (d 
                     (let loop ((i 0) (d 0.0))
                      (if (fx= i (vector-length o))
                          d
                          (loop (fx+ i 1)
                                (fp+ d
          ...
          (if (< min1 (* min2 threshold))
              (list object-keypoint min-keypoint)
              #f))))
       object-sift)))
